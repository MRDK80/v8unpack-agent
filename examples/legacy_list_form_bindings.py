...
Статус: historical (#252). Файл сохранён как исторический пример: он
документирует layout legacy `ФормаСписка` / `ФормаВыбора` и связку
`source` -> `data_path`, полученную безопасным fallback #103 и #107 на
реальной выгрузке. Актуальная замена для диагностики неиндексируемых
форм — `examples/unindexed_forms_report.py`: он самодостаточен, даёт
RC=0 без аргументов и покрывает шесть категорий `UnindexedReason`.
Контракт элемента `TabularFieldColumn` описан в `docs/elem_parser.md`.
В автоматический регрессионный прогон файл не входит.

Категория: пример на реальной выгрузке.
Входные данные: FORM_DIR — каталог формы из выгрузки v8unpack.
Ожидаемый результат: обезличенный агрегат в stdout; коды возврата —
RC=0, если форма проиндексирована (elem_index_ok=True);
RC=1, если elem_index_ok=False — это штатный признак
неиндексируемой формы, агрегат и диагностика причины #105
