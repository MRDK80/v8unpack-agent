...
Единая точка входа для обработки уже распакованной выгрузки: один прогон
даёт один post-run report и один exit code.

```bash
v8unpack-agent-run <корень_выгрузки> --report-path <путь_к_отчёту>
```

Равнозначный вызов без установки пакета:

```bash
python -m v8unpack_agent.cli <корень_выгрузки> --report-path <путь_к_отчёту>
```

`--report-path` обязателен и не имеет значения по умолчанию: прогон без
записи отчёта невозможен. Каталог-родитель должен существовать заранее.

Коды возврата: `0` успех, `2` ошибка аргументов, `3` degraded,
`4` управляемая фатальная ошибка, `5` ошибка записи отчёта, `6` фатальная
ошибка вместе с отказом записи. Degraded считается неуспешным завершением
процесса.

Модуль `v8unpack_agent.runner` — чистая библиотека без побочных эффектов;
разбор аргументов, запись отчёта и владение exit code сосредоточены в
`v8unpack_agent.cli`. Подробности: [docs/runner.md](docs/runner.md).
...
- [`examples/reference_only_compare.py`](examples/reference_only_compare.py) — кросс-конфигурационная проверка класса `reference_only`: подключается к отчёту флагом `--compare-root`, публикует только ранги `P01…` без UUID и локальных имён, самопроверка `--selftest` работает без выгрузки (#164). Результат #164: `partially_confirmed` — 6 UUID / 868 вхождений подтверждены, отчёт [`docs/research/platform_types_cross_config_issue164.md`](docs/research/platform_types_cross_config_issue164.md).
