...
| `form_context` | `FormContext` + `build_form_context(form_entry, unpacked_root)` + `to_llm_prompt_fragment(context, max_chars=-1)` — материализация формы: BSL-текст, `FormSummary` и компактные метаданные в одном объекте, готовом для вставки в промпт; по умолчанию контекст не обрезается, положительный лимит ограничивает длину. Символы экспортируются из корня пакета лениво (#124). → [подробнее](docs/form_context.md) |
