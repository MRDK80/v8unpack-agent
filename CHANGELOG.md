...
- `project.version` зафиксирована как `0.1.0`; тег релиза — `v0.1.0`.
  Статическое поле остаётся единственным источником версии.
- Runtime-зависимость остаётся индексной `v8unpack>=1.2.13`. Нижняя граница
  подтверждена независимым probe в чистом окружении: установка с production
  PyPI без cache, версия дистрибутива `1.2.13`, `pip check` RC=0, импорт из
  `site-packages`, тип `ExternalReport` зарегистрирован в
...
- `examples/unresolved_refs_report.py`: строка «нет» в разделе
  «## слоты иного layout» печаталась всегда, потому что `list.extend()`
  возвращает `None` и идиома `extend(...) or append("нет")` выполняла
  `append` безусловно. В #264 выражение было разложено на два statement
  без изменения вывода; теперь ветвление явное: при непустом наборе
  перечисляются слоты, при пустом печатается «нет».
- Удалён комментарий из #264, объяснявший сохранение дефекта: он
  утверждал, что вывод зафиксирован тестом #251, тогда как
  `unresolved_refs_report.py` в кейсы
  `tests/test_examples_determinism_issue251.py` не входит.

### Проверено

- Непустой набор слотов: перечисление без строки «нет». Набор создан
  подстановкой существующего слота вне репозитория, поскольку на реальных
  выгрузках слотов иного layout с поддержкой контроля не найдено (#143).
- Пустой набор слотов на реальной выгрузке: вывод примера побайтово совпал
  с выводом до правки.
- Идиома `extend(...) or append(...)` в остальных файлах `examples/`
  отсутствует — проверено разбором AST, а не однострочным grep.

### Не изменялось

- Ожидания `tests/test_examples_determinism_issue251.py`: девять кейсов —
  восемь самодостаточных примеров и `reference_only_compare.py --selftest`;
  затронутого файла среди них нет.
- Формат строки слота, детерминированный порядок `sorted(layout)`
  и остальные разделы отчёта.
- Production-код `v8unpack_agent/`, публичный API, `pyproject.toml`,
  workflow и контрактные блоки примеров (#246).

## Область mypy синхронизирована с конфигом (#268)

### Изменено

- `[tool.mypy] files` в `pyproject.toml` расширен до
  `["v8unpack_agent", "examples"]`: конфигурация стала единственным источником
  истины об области статической проверки типов.
- CI-шаг `Mypy` в job `lint / ruff + mypy` выполняет команду `mypy` без
  аргументов вместо `mypy v8unpack_agent examples`. Аргументы удалены
  полностью, потому что явные аргументы переопределяют `files`, а не
  дополняют его: при их сохранении конфигурация осталась бы декоративной.
- Расхождение до правки: `mypy` — RC=0, 25 файлов; `mypy v8unpack_agent
  examples` — RC=0, 40 файлов, то есть локальный голый прогон проверял
  меньше, чем CI. После правки обе команды дают RC=0 и 40 файлов.
- Документация приведена к фактической команде: `CONTRIBUTING.md` и
  `examples/README.md` указывают команду `mypy`, источник области
  `[tool.mypy] files` и ожидаемое число файлов.

### Не изменялось

- Число обязательных CI-чеков — шесть; имена jobs, матрица pytest, шаг Ruff
  и триггеры workflow сохранены.
- Production-код `v8unpack_agent/`, публичный API и содержание примеров.
- Правила строгости mypy: `strict`, `disallow_untyped_defs` и подобные
  не добавлялись — расширение строгости остаётся отдельным решением.
- Версии линтеров и набор правил `ruff` (пин и обновление — решение #161).
- Исторические таблицы замеров в `docs/IMPLEMENTATION_STATUS.md` и
  `docs/documentation_audit_issue245.md`.

## Каталог examples/ включён в область mypy (#264)

### Изменено

- CI-шаг `lint / ruff + mypy` выполняет `mypy v8unpack_agent examples`
  вместо `mypy v8unpack_agent`: каталог примеров ранее в область проверки
  не входил, из-за чего 15 исполняемых примеров публичной витрины API
  оставались непроверяемыми при зелёном CI. Число обязательных CI-чеков
  не изменилось — шесть.
- `examples/`: сняты 27 ошибок mypy в 6 файлах. Решение принято по замеру
  на `abee03b82e1605902460c37eb353b195e7678478`, а не по цифре «29 ошибок
  в 7 файлах» из тела issue: та снята на `917b937`, до merge #253, где
  добавлены две аннотации `Counter[str]` и переписан
  `extract_skd_queries.py`.
- Характер правок: аннотации `Counter[str]`, `list[dict[str, Any]]` и
  `dict[str, Any]` вместо вывода типа до `object`; сужение
  `ZeroBindingReason | None` перед обращением к `.value`;
  `cast(Iterable[Any], ...)` вместо неверного
  `type: ignore[union-attr]`; возврат `Any` у `_clone_entry`, где
  структурный контракт проверяет сама `object_json_path`; `len([...])`
  вместо `sum(1 for ...)` в двух местах, где mypy 2.3.1 выбирает
  неподходящую перегрузку `sum`.

### Не изменялось

- Наблюдаемый вывод примеров: `tests/test_examples_contract_issue246.py`
  (16 проверок) и `tests/test_examples_determinism_issue251.py`
  (36 проверок, побайтовое сравнение stdout) остаются зелёными,
  `reference_only_compare.py --selftest` даёт RC=0.
- Production-код `v8unpack_agent/`, публичный API, `pyproject.toml`,
  состав каталога примеров и их контрактные блоки (#246).
- CRLF-переводы строк `examples/extract_skd_queries.py`.
- Исторические таблицы замеров в `docs/IMPLEMENTATION_STATUS.md` и
  `docs/documentation_audit_issue245.md`: они фиксируют состояние на
  момент прошлых задач и не описывают текущую область.

### Задокументировано

- Из трёх вариантов, предложенных в #264, выбран первый: область
  расширена, а не зафиксирована как непроверяемая. Основание фактическое —
  #253 показал, что непроверяемый пример может разбирать несуществующий
  носитель, и ни один из шести CI-чеков этого не поймал.
- Известный дефект вывода `examples/unresolved_refs_report.py`: в разделе
  «слоты иного layout» строка «нет» печаталась всегда, поскольку
  `list.extend()` возвращает `None` и идиома `extend(...) or append(...)`
  никогда не работала как fallback. Вывод сохранён без изменений, так как
  зафиксирован тестом #251; исправление смысла вынесено отдельной issue
  с родителем #157.

## Пример extract_skd_queries.py переведён на публичный API (#253)

### Исправлено

- `examples/extract_skd_queries.py`: запросы СКД извлекает публичная функция
  `v8unpack_agent.skd_extractor.extract_skd_queries()`. Собственный разбор
  файла `metadata` регулярными выражениями удалён вместе с `_unescape_1c()`,
  `find_metadata_file()` и `extract_queries()`.
- Основание решения — проверка на реальной выгрузке `.erf`, а не
  предположение: файла `metadata` в ней нет вовсе, схема компоновки данных
  лежит в контейнере `Template/<ИмяСхемы>/Template.bin`. Прежний разбор не
  находил носитель и завершался с RC=1; публичная функция извлекает набор
  данных без предупреждений. Расхождение вывода «до/после» объясняется
  сменой носителя и признано корректным.

### Не изменялось

- CLI `--unpack-dir`, `--output`, `--report-name`, структура вывода
  `{"report": ..., "datasets": [...]}` и коды возврата RC=0/1/2: RC=1
  сохранён для отсутствующего каталога выгрузки и для выгрузки без
  контейнера схемы, RC=0 — для записанного JSON, включая пустой набор.
- CRLF-переводы строк файла примера.
- Production-код `v8unpack_agent/`, публичный API, workflow и
  `pyproject.toml`.

### Задокументировано

- `examples/reference_only_compare.py` сохраняет собственную реализацию
  осознанно: модуль не читает выгрузку, агрегаты ему передаёт
  `unresolved_refs_report.py`, а `re`, `json` и `sha256` обслуживают
  вердикты, ранги и `anonymity_guard` методики #164. Воспроизводимой через
  `scan_forms` / `object_decoder` части в файле нет. Добавлены две
  аннотации `Counter[str]` в `class_counts()`, снимающие ошибки mypy
  `var-annotated` без изменения наблюдаемого поведения.
- Побочный эффект публичной функции: помимо `--output` она всегда пишет
  `skd_queries.json` в корень переданной выгрузки.

## Обезличенный вывод путей в legacy_list_form_bindings.py (#262)

### Исправлено

- `examples/legacy_list_form_bindings.py`: ссылка на форму больше не
  печатается абсолютным путём. В текстовом режиме и в поле `form_dir`
  режима `--json` выводится путь относительно корня выгрузки в POSIX-виде,
  а без указанного корня — только имя каталога формы.
- Абсолютный путь формы в тексте детали `classify_unindexed_form()` (#105)
  вычищается на стороне примера подстановкой по известному `form_dir`,
  без регулярных выражений по произвольному тексту.
- Сообщение «Директория формы не найдена» печатает путь в том виде, в
  котором он передан аргументом, без разворачивания в абсолютный.

### Добавлено

- `examples/legacy_list_form_bindings.py`: опциональный аргумент
  `--export-root` — корень выгрузки, относительно которого печатается
  ссылка на форму. Позиционный `FORM_DIR`, формат вывода и коды возврата
  RC=0/1/2 (#261) не изменились.
- В блок контракта module docstring добавлена строка об отсутствии
  абсолютных путей в выводе.

### Не изменялось

- Production-код `v8unpack_agent/`, публичный API, workflow и
  `pyproject.toml`. Пример остаётся historical (#252) и real-data: на
  синтетику не переводится и в автоматический регрессионный прогон не
  входит.

## Решение по historical-примеру legacy_list_form_bindings.py (#252)

- `examples/legacy_list_form_bindings.py` сохранён как исторический пример.
  В module docstring добавлен блок `Статус: historical (#252)` со ссылками на
  актуальные артефакты: `examples/unindexed_forms_report.py` для диагностики
  неиндексируемых форм и `docs/elem_parser.md` для контракта элемента
  `TabularFieldColumn`.
- Основание решения — фактическое перекрытие сценария, а не предположение.
  Диагностическая часть (#105) полностью покрыта `unindexed_forms_report.py`:
  он самодостаточен, даёт RC=0 без аргументов и разбирает шесть категорий
  `UnindexedReason`. Демонстрация подтверждённых привязок колонок (#103, #107)
  с полем `source` в каталоге примеров уникальна: ни один другой пример её не
  воспроизводит, поэтому удаление файла отклонено. Успешная ветка разбора
  защищена регрессией `tests/test_elem_parser_issue103.py`, поэтому перевод
  примера на синтетические данные также отклонён.
- `examples/README.md`: строка примера помечена historical с указанием замены.
- Состав каталога примеров не изменился. Production-код `v8unpack_agent/`,
  публичный API, workflow и `pyproject.toml` не изменялись.
...
- **Документация: README сведён к landing page** (issue #245): справочники и
  ...
  контракт получил одно место. Размер README — 14830 bytes против 30123.
  Python quick start переписан на `FormBinSource` и двухаргументный
  `FormUnpacker`: прежний пример передавал трёхаргументную функцию напрямую в
  `unpack_all_forms()` и был несовместим с контрактом после #226. Разведены
  три версии схем: `FormsIndex` 2, `FormScanIndex` 2 при чтении legacy 1,
  post-run report 1. Добавлена ссылка на `docs/run_report.md`, который ранее
  не имел входящих ссылок. Исправлена команда установки тестовых
  зависимостей. Из `docs/scan_forms.md` удалено упоминание внутреннего
  репозитория. В `docs/form_classifier.md` метрика покрытия `data_path`
  выделена как канонический раздел, устранены дублированный абзац и
  ошибочный итог при слагаемых 17 + 26 + 2. Аудит всех 28 Markdown-файлов —
  `docs/documentation_audit_issue245.md`. Production-код не изменялся.

- **`unindexed_forms_report`: исправлено synthetic-покрытие динамического
  источника** (issue #235): UUID-заглушка заменена публичным package-owned
  marker; распределение причин закреплено детерминированным тестом.
...
- **`FormScanIndex` сериализует только относительные пути** (issue #239):
  ...
  `scan_root`, которое само в payload не попадает. Добавлен
  `schema_version = 2`; payload без ключа считается legacy-схемой 1 с
  абсолютными путями и читается без перезаписи (read-compatible,
  write-new). `FormScanIndex.load()` принимает необязательный `root` и
  разрешает относительные пути относительно него, поэтому индекс
  переносится между каталогами и машинами. `check_drift()` разрешает пути
  baseline через `cf_export_root`, а `FormRouter` принимает `scan_root`.
  Публичная сигнатура `scan_forms()` не изменилась. Контроль на реальной
  выгрузке: 2216 форм, 0 вхождений абсолютного корня, 0 литеральных
  разделителей Windows.
- **`scan_forms()` проверяет корень выгрузки**: несуществующий путь и путь к файлу теперь немедленно вызывают `NotADirectoryError`; валидная пустая директория по-прежнему возвращает пустой индекс. При ошибочном корне `save_to` не создаётся и не перезаписывается (issue #234).
...
- **`scan_forms`: машинные коды предупреждений** — стабильный суффикс
  `[code=UPPER_SNAKE]`, публичный parser `scan_warning_code()`, legacy-compatible
  save/load и документированный перечень кодов (issue #167).
