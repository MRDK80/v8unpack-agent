...
## Разрешение путей baseline (issue #239)

Начиная со схемы 2 индекс хранит относительные пути, поэтому `check_drift()`
разрешает их относительно `cf_export_root`. Это касается двух проверок:

- legacy-fallback по `bsl_mtime`, когда в записи нет `bsl_sha256`;
- классификации elem-only форм, где проверяется наличие BSL на диске.

Следствия:

- одинаковое дерево под разными корнями не даёт ложный drift;
- реальное изменение файла по-прежнему обнаруживается;
- baseline legacy-схемы с абсолютными путями читается без изменений.

`FormRouter` принимает необязательный `scan_root`. Он нужен, когда в
`reindex()` передаются записи свежего `scan_forms()` с абсолютными путями:
без корня портируемый индекс записать нельзя.
