...
- Верификация #103: 487 тестов; полный проход по 2216 формам — OK 2139,
...
- Верификация #105 на 2216 формах: проиндексировано 2139 (96.5%),
...
- Ограничение: 80 форм в `unknown` — обычные формы с бинарной разметкой,
  которую `elem_parser` не читает (49 из них подняты через PR #102;
  225 `ФормаСписка`/`ФормаВыбора` остаются ERROR до реализации #103).
...
- #107 — категория B (48 форм): UUID колонок `TabularField` не найдены среди
  реквизитов владельца. Гипотеза — колонки ссылаются на реквизиты табличных
  частей (`TabularSections[].Properties`) или на независимые значения.
- #108 — категория A (12 форм): `object_decoder` не распознал layout владельца
  (`ChartOfCharacteristicType`), либо владельца нет вовсе (`CommonForm`) —
  нужна отдельная ветка `NO_OWNER_OBJECT`.
- #109 — категория C (17 форм): формы без виджетов данных
  (`ФормаПечати`, `ФормаНастройки`, диалоги) — помечать как `service`,
  а не `unknown`, чтобы не искажать метрику покрытия.
...
- На проверенной конфигурации остаются 77 форм без достоверно извлечённой
