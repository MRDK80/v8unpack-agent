...
## Публикация результатов roadmap #157

22 сентября 2026 года результаты эпика #157 опубликованы на Инфостарте:
[«Конвейер форм 1С отчитался зелёным. Верим?»](https://infostart.ru/1c/articles/2794715/).
Статья связывает в один проверяемый контракт валидацию входа, полноту
обнаружения форм, сохранение недоказанных ссылочных типов, машинные коды
завершения и переносимость сериализованных индексов.

Публикация не расширяет фактические возможности пакета. Встроенного
production-распаковщика одиночного `Form.bin` по-прежнему нет, метрика
нерезолвленного остатка ещё не входит в post-run report (#288), а
LLM-фрагмент не содержит отдельного признака недоказанного ссылочного типа
(#289).
