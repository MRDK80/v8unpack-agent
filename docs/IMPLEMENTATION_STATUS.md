...
| `to_llm_prompt_fragment(context, max_chars=8000)` | ✅ | #77 |
...
- `to_llm_prompt_fragment` обрезает результат последним шагом, поэтому
  лимит `max_chars` нарушить невозможно; `max_chars <= 0` даёт пустую строку.
