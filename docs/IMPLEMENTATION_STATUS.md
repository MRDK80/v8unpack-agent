...
- **`ФормаСписка` / `ФормаВыбора`: TabularField-структура с числовыми индексами
  колонок (#103).** `extract_legacy_form_elements` не читает форматы без тега
  `"14"` на уровне виджета. 225 форм УТ 10.3 остаются `ERROR`. Требует
  кросс-чтения `form/*.json` и `../Object.json` через `decode_object_attributes`.
...
- `ФормаСписка` / `ФормаВыбора` (225 форм) — обычные формы с TabularField и
  числовыми индексами колонок. `elem_parser` не читает этот формат (#103).
