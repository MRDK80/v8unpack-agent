...
- Оставшиеся 77 неиндексируемых legacy-форм: структуры без подтверждённых
  UUID-привязок `TabularField`, пустая карта реквизитов владельца и формы
  других типов — исследование и классификация в #105.
...
- После реализации #103 на проверенной конфигурации остаются 77 форм без
  достоверно извлечённой разметки. Они сохраняют `elem_index_ok=False`;
  нестрогий рекурсивный поиск UUID намеренно не применяется (#105).
