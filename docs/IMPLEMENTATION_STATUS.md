...
- Стандартные реквизиты (`Код`, `Наименование`, `Родитель`) в **управляемых**
  формах привязки не получают: их UUID отсутствуют в метаданных объекта (#84).
  В обычных формах те же реквизиты разрешаются — привязка идёт по имени.
