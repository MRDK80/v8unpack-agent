...
## Актуальный срез эпика #157

Перед началом #210 в
[карточке roadmap](https://github.com/MRDK80/v8unpack-agent/issues/157)
были отмечены выполненными 49 из 50 пунктов. Последним открытым пунктом была
#210 — финальная интеграционная проверка документации и закрытие roadmap.
Итоговое состояние roadmap фиксируется в GitHub после merge и успешного
post-merge CI нового SHA `main`.

По локальной истории `main` реализованы ключевые части контура: опись форм,
разбор элементов и реквизитов, разрешение ссылочных типов, контекст,
индексы и контроль дрейфа, production runner с отчётом и CLI. Стабильная
версия `0.1.0` выпущена. Связанные задачи #165, #198, #245 и #246 вошли в
текущий код и документацию; исследовательские разделы ниже сохраняют
состояние на дату соответствующего измерения.

Для #165 текущий резолвер использует индекс объектов конфигурации, затем
`PLATFORM_REFERENCE_TYPES` с 13 доказанными именами XDTO. Один доказанный
UUID без публикуемого имени и остальные неизвестные UUID остаются `Ref#uuid`.
На измеренной выгрузке A после исправления метрики #278 покрытие составляет
15448 / 15723 применимых вхождений (98.2510%). Подробности — в
[`scan_forms.md`](scan_forms.md) и
[`research/platform_type_names_issue165.md`](research/platform_type_names_issue165.md).

Состояние проверено по карточке эпика и локальной истории `main`.
...
- Гарантия ленивости корневого импорта (issue #134, часть B): `import
  ...
  816 passed локально.
- Issue #140 закрыл последний eager-путь корневого импорта: `form_classifier`
  и `form_router` переведены в ленивые группы `__getattr__`, вместе с
  классификатором ушёл транзитивный `coverage_metric` (`DATA_ELEMENT_TYPES`).
  После чистого импорта в `sys.modules` остаётся 2 модуля пакета вместо 5.
  Медиана холодного импорта снизилась с 10 679,1 до 278,1 µs (−97,4 %) на
  9 прогонах в отдельных процессах, Python 3.12.3, Linux. Публичная
  поверхность не сокращена: `__all__` — те же 42 имени, deprecated-шимы не
  переупорядочены. `form_paths` оставлен eager осознанно: имя одновременно
  подмодуль и публичная функция, поэтому ленивое разрешение подменило бы
  функцию модулем при прямом импорте подмодуля. Проверка —
  `tests/test_init_lazy_imports_issue140.py`.
...
| #165 | на момент исследования оставался заблокированным; позже реализована статическая таблица, см. актуальный срез выше |
