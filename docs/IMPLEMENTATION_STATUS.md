...
- **#89** — closed (PR #115): сегментные цепочки `data_path` управляемых форм.
  Остаются 42 формы с непустой картой и нулевой привязкой — у них блока
  привязки в записях элементов нет вовсе либо стоит `["1", "0"]`; привязка
  для них не создаётся осознанно, точная классификация вынесена отдельно.
