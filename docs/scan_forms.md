...
попасть в этот индекс через расширение обхода. После #165 резолвер отдельно
использует статическую таблицу `PLATFORM_REFERENCE_TYPES`; порядок ступеней и
границы таблицы описаны ниже.
...
не человекочитаемое имя типа. Это исторический результат #164: после проверки
третьей конфигурации и отдельного эксперимента с именами реализована таблица
#165. Текущий результат и ограничения приведены в разделе «Двухступенчатая
резолюция ссылочных типов» ниже.
...
| Кандидат платформенного типа отсутствует в файлах выгрузки и в `PLATFORM_REFERENCE_TYPES` | Недостаточно доказательств имени | Сохранить fallback; не расширять таблицу без доказательства |
...
## Контракт корня выгрузки (issue #234)

`cf_export_root` должен существовать и быть директорией. Несуществующий путь
и путь к обычному файлу приводят к `NotADirectoryError` до обхода дерева и до
сохранения `save_to`. Существующая пустая директория остаётся валидным входом:
`scan_forms()` возвращает пустой `FormScanIndex` с `total = 0`. Standalone CLI
сообщает ту же ошибку через `argparse` и завершается с кодом 2.

## Портируемая сериализация индекса (issue #239)

`FormScanIndex` различает runtime-семантику путей и семантику сериализации.

Runtime:

- `form_path`, `bsl_path`, `json_path` и `form_elem_path` — абсолютные;
- `elem_json_path` — относительный к корню выгрузки (контракт issue #57);
- корень хранится в поле `scan_root`, которое заполняет `scan_forms()`.

Сериализация:

- все path-поля записываются относительно `scan_root` через прямой слэш;
- абсолютные пути, буква диска, ведущий слэш и `..` в payload не попадают;
- `scan_root` не сериализуется ни отдельным полем, ни внутри записей;
- предупреждения `scan_warnings` также не содержат корня.

Поле `scan_root` обязательно, если хотя бы один путь абсолютный. Без него
`to_dict()` выбрасывает `ValueError`. Путь вне корня — тоже явная ошибка:
корень не угадывается по общему префиксу.

### Версия схемы и миграция

Payload схемы 2 содержит ключ `schema_version`. Payload без этого ключа
считается legacy-схемой 1 с абсолютными путями. Политика — read-compatible,
write-new: старый индекс читается, но перезаписывается уже в новом формате;
автоматическая миграция чужих файлов не выполняется. Неизвестная версия
отклоняется `ValueError`.

### Загрузка относительно другого корня

```python
index = FormScanIndex.load(Path("forms_scan_index.json"), root=export_root)
```

С аргументом `root` относительные пути схемы 2 разрешаются в абсолютные
относительно переданного корня, поэтому индекс, снятый на одной машине,
используется на другой без правки payload. Без `root` пути остаются
относительными. Абсолютные пути legacy-схемы не изменяются.

Значение `elem_json_path` остаётся относительным в любом случае — это
неизменённый контракт issue #57.

## Двухступенчатая резолюция ссылочных типов (#165)

`FormScanIndex.resolve_reference_type` отвечает на вопрос «какое имя типа
соответствует UUID» в фиксированном порядке:

1. `reference_types` — индекс, построенный обходом выгрузки: имена объектов
   самой конфигурации в стиле `CatalogRef.<Имя>`;
2. `PLATFORM_REFERENCE_TYPES` — статическая таблица платформенных типов из
   модуля `v8unpack_agent.platform_reference_types`: 13 записей вида
   `prefix:LocalName` (каноническое машинное имя XDTO);
3. `None` — резолвер не знает имени, и `object_decoder` сохраняет значение
   `Ref#uuid` без потери идентификатора.

Первая ступень приоритетнее статики: объекты конфигурации никогда не
перекрываются платформенной таблицей. Непустое пересечение ключей таблицы с
`reference_types` считается ошибкой таблицы, а не поводом менять приоритет.

### Границы таблицы

- Таблица заполняется вручную по доказательствам issue #164 и отчёту
  `docs/research/platform_type_names_issue165.md`; автогенерация и чтение
  выгрузки запрещены.
- `PLATFORM_TYPES_WITHOUT_XDTO_NAME` содержит один UUID доказанного объёма,
  у которого машинного имени нет по прямому измерению. Он не участвует в
  резолюции и существует для трассируемости и тестов. Инвариант: `13 + 1 = 14`
  при пустом пересечении множеств.
- UUID, подтверждённые только в одной выгрузке, и побочные наблюдения вне
  целевого набора в константы не входят.
- Таблица не сериализуется в составе индекса: schema v2 не изменяется.
