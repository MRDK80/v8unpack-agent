...
Как следствие, входной контракт `resolve_data_path` полностью совпадает
с контрактом декодера: принимается путь к реальному object JSON в raw-header
layout, то есть с верхнеуровневым ключом `header`.

Нормализованный layout (готовые списки `Properties` в корне JSON) входным
форматом **не является** и после #160 это зафиксировано явно: декодер отклоняет
такой файл с `HEADER_MISSING`, а `resolve_data_path()` возвращает
`ResolvedBinding` с `resolved=False`, `value_type=None` и `synonym=None`.
Исключение наружу не выходит, тип и синоним не выдумываются. Passthrough плоской
структуры не выполняется: потребитель, у которого нормализованные данные уже
есть, использует их напрямую, а не записывает во временный object JSON для
повторного декодирования. См. «Поддерживаемые входные layout» в
...
После #160 отказ от нормализованного входа закреплён как контракт, а не как
побочный эффект #148: обратной поддержки плоских ключей `Properties` /
`Attributes` в корне JSON нет и не планируется.
