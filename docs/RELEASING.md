...
Единственный источник версии — статическое поле `project.version` в `pyproject.toml`. Git-тег не вычисляет версию, а подтверждает её и обязан иметь вид `v<version>`. Например, версии `0.1.0` соответствует тег `v0.1.0`. Несовпадение блокирует workflow до upload.
...
3. Установить `project.version` равным выпускаемой версии, например `0.1.0`.
...
10. После отдельного подтверждения создать защищённый тег, точно соответствующий версии из `pyproject.toml` и начинающийся с `v`.
