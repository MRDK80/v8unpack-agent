...
`to_llm_prompt_fragment(context, max_chars=-1, *, max_tokens=None, count_tokens=None)`

Символьный режим — по умолчанию. Без `max_tokens` и `count_tokens` результат
символ в символ совпадает с поведением #77, включая семантику `max_chars`.

Токенный режим включается только парой аргументов:

- `max_tokens` и `count_tokens` передаются вместе. Передача только одного
  из них — `ValueError`. Частичная конфигурация не игнорируется молча.
- `max_tokens <= 0` возвращает пустую строку, как и `max_chars = 0`.
- Токенайзер передаёт вызывающая сторона; пакет не зависит от `tiktoken`
  и других библиотек.

Оба бюджета действуют одновременно. Итог — наибольший префикс из целых
строк санитизированного фрагмента, для которого:

- длина не превышает длину результата символьного режима с тем же
  `max_chars`, то есть `len(result) <= max_chars`;
- `count_tokens(result) <= max_tokens`.

Граница строк: в токенном режиме фрагмент никогда не обрывается посреди
строки. Строка отрицательного знания #141 вида
`data_path: ...; status: ...; reason: ...` включается целиком либо
исключается целиком. Порядок секций не меняется: `## SUMMARY` раньше `## BSL`.
Если бюджета не хватает, последние секции выпадают целиком; выбор секций —
задача #146.

Санитизация #142: токенный режим работает только с текстом, который вернул
символьный путь, уже прошедший `sanitize_diagnostic`. Отдельного канала к
исходным данным нет.

Сбой счётчика: если `count_tokens` бросает исключение или возвращает не
`int`, `bool` или отрицательное число, функция не падает и возвращает
fail-safe fallback — префикс целых строк в пределах символьного бюджета.
Исходное исключение не пробрасывается. Токенная гарантия в этом случае не
действует: её невозможно проверить без рабочего счётчика.

Пример с фейковым счётчиком:
