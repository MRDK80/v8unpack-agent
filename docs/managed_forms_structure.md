...
Подтверждено на синтетических тестовых данных: issue #55 (июль 2026).
...
Управляемая форма конфигурации — 4-уровневый layout:
...
Примеры относительных путей:
    ...
    Catalog/Банки/CatalogForm/ФормаЭлементаУправляемая/ФормаЭлементаУправляемая.elem.json
    external_managed/ВнешнийОтчетУправляемый/ReportForm/ФормаОтчетаУправляемая/ФормаОтчетаУправляемая.elem.json
    external_managed/ВнешняяОбработкаУпр/Form/ФормаВнешняяУправляемая/ФормаВнешняяУправляемая.elem.json
...
| `Form`           | Формы обработок, общие формы |
| `ReportForm`     | Формы отчётов |
...
- `elem_json_path` → `<object_type>/<object_name>/<Container>/<form_name>/<form_name>.elem.json`
