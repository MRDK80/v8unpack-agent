...
Бакет заполняется из `data_path` элементов, которые `parse_elem_json` декодирует
и для обычных, и для управляемых форм (issue #85). Ранняя реализация на
`managed_form_adapter` давала `relations = 0` — сейчас на реальной выгрузке
`Catalog/Банки` обычная `ФормаЭлемента` даёт 8 связей, `ФормаСписка` — 4.

Пустой `relations[kind=data]` при непустом `elements` — законный результат для
...
- Одноимённые записи из `data` и `props` схлопываются ещё в `parse_elem_json`,
  поэтому в `elements` один элемент представлен один раз даже там, где имя
  реквизита формы совпадает с именем элемента (`СправочникСписок`).
