...
------------------------------
``import v8unpack_agent`` уже сейчас (до #124) загружает ``v8unpack_agent.elem_parser``
транзитивно через верхнеуровневые импорты пакета. Это существующее поведение ``main``
и оно вне scope #124, поэтому тесты фиксируют то, что действительно относится к
этой задаче: ``form_context`` не импортируется до первого обращения к новым именам.
"""
